...
public class PaymentService {
    ...
    public PaymentResponse create(Jwt jwt, Long orderId, CreatePaymentRequest request) {
        ...
        PaymentAttemptService.PreparedAttempt attempt =
                attemptService.prepare(user.getId(), orderId, request.method());
        if (attempt.completed()) return PaymentResponse.from(attemptService.find(attempt.paymentId()));
        try {
            PixPaymentGateway.Result result = pixPaymentGateway.create(new PixPaymentGateway.Request(
                    attempt.orderId(), attempt.paymentId(), attempt.amount(), attempt.payerEmail(),
                    attempt.idempotencyKey()));
            return PaymentResponse.from(attemptService.complete(attempt.paymentId(), result));
