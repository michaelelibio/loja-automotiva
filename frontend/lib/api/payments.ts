...
export async function createPixPayment(orderId: number): Promise<Payment> {
  ...
  if (!response.ok) {
    ...
    throw new PaymentApiError(String(object?.message || object?.error || 'Não foi possível gerar o PIX.'), response.status);
  }
